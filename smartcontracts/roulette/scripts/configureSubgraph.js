...
// ============================================
// CONFIGURATION - Update these for different contracts
// ============================================
const CONFIG = {
  CONTRACT_ADDRESS_VAR: 'NEXT_PUBLIC_ROULETTE_CONTRACT_ADDRESS',
  START_BLOCK_VAR: 'ROULETTE_START_BLOCK',
  NETWORK_VAR: 'NETWORK'
};
// ============================================
...
function validateEnv(env) {
  const required = [CONFIG.CONTRACT_ADDRESS_VAR, CONFIG.START_BLOCK_VAR, CONFIG.NETWORK_VAR];
...
function updateSubgraphConfig(env) {
  ...
  content = content.replace(
    ...
    `address: "${env[CONFIG.CONTRACT_ADDRESS_VAR]}"`
  );
  
  // Replace start block
  content = content.replace(
    /startBlock: \d+/,
    `startBlock: ${env[CONFIG.START_BLOCK_VAR]}`
  );
  
  // Replace network
  content = content.replace(
    /network: [a-zA-Z0-9-]+/,
    `network: ${env[CONFIG.NETWORK_VAR]}`
  );
...
function configure() {
  ...
  try {
    ...
    console.log(`📝 Contract Address: ${env[CONFIG.CONTRACT_ADDRESS_VAR]}`);
    console.log(`📝 Start Block: ${env[CONFIG.START_BLOCK_VAR]}`);
    console.log(`📝 Network: ${env[CONFIG.NETWORK_VAR]}`);
