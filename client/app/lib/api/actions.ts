...
> => {
  try {
    ...
    return {
      ...
      feeCollectorAddress: result[4],
      feeBpsPercentage: result[5] / 100,
      minBetAmount: weiToUsdc(result[6]),
      maxBetAmount: weiToUsdc(result[7]),
      maxOpenRoundsPerUser: Number(result[8]),
      minOpenTimeInSeconds: Number(result[9]),
      minLockTimeInSeconds: Number(result[10]),
      advanceCooldownInSeconds: Number(result[11]),
      priceMaxAge: Number(result[12]),
      paused: result[13],
    };
  } catch (error) {
